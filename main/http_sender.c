...
/**
 * Loads the Receiver IP address from NVS.
 * This IP is set via the Provisioning Web Portal.
 */
static esp_err_t load_receiver_settings() {
    nvs_handle_t h;
    ...
    if (err == ESP_OK && strlen(pc_ip) > 0) {
        ...
        ESP_LOGI(TAG, "Settings Loaded! Target URL: %s", dynamic_url);
        return ESP_OK;
    }
    return ESP_FAIL;
}
...
static void http_sender_task(void *arg) {
    ...
    // 2. Wait for NVS settings to be provided via the web portal
    while (load_receiver_settings() != ESP_OK) {
        ESP_LOGW(TAG, "Receiver IP not set in NVS. Waiting for provisioning...");
        
        // Fast Blink indicates "Stationary mode: Waiting for configuration"
        for(int i = 0; i < 5; i++) {
    ...
    // 3. Configuration Found: Set LED to SOLID ON
    set_led(1);

    // 4. Initialize HTTP client with the dynamic URL from NVS
    ...
    while (1) {
        ...
        if (batch_count > 0 && (batch_count >= BATCH_SIZE || (now - last_flush) > (FLUSH_MS * 1000))) {
            ...
            for (int i = 0; i < batch_count; i++) {
                ...
                // Correctly mapping the 13 arguments to the format string
