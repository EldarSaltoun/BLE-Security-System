@echo off
title BLE Security System — Distributed Launcher
color 0b

REM === CONFIGURATION ===
set PYTHON=python
set PROJECT_DIR=%~dp0
set SERVER_URL=http://192.168.1.19:8000
set STREAM_URL=http://127.0.0.1:8000/api/ble/stream

echo ==================================================
echo   BLE SECURITY SYSTEM : MINIMAL-ESP / MAX-PYTHON  
echo ==================================================
echo.

REM === STEP 1: Start Fast-Ack Receiver ===
echo [1/2] Starting High-Throughput Receiver on 192.168.1.19...
start "BLE_SERVER_RECEIVER" cmd /k ^
  "cd /d "%PROJECT_DIR%" && %PYTHON% pc_receiver.py"

REM === STEP 2: Wait for Server Bind ===
echo.
echo Waiting for server to initialize...
timeout /t 5 >nul

REM === STEP 3: Start Real-Time Popup GUI ===
echo [2/2] Starting Live Presence Popup...
REM Note: Removed --mfg-db to fix the argparse error
start "BLE_GUI_POPUP" cmd /k ^
  "cd /d "%PROJECT_DIR%" && %PYTHON% ble_popup.py --url %STREAM_URL%"

echo.
echo --------------------------------------------------
echo BLE System initialized successfully.
echo - IP Address: 192.168.1.19
echo - Fast-Ack Mode: ENABLED
echo.
echo You may now power on the ESP32-S3 boards.
echo --------------------------------------------------
pause