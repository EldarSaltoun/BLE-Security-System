        ...
        if not os.path.exists(self.csv_log):
            with open(self.csv_log, "w", newline="", encoding="utf-8") as f:
                ...
                writer.writerow([
                    ...
                    "scanner", "timestamp_epoch_us", "payload_sig"
                ])
    ...
    def ingest(self, mac, rssi, channel, name, txpwr, mfg_id, adv_len,
        ...
        with self.lock:
            self.devices[mac] = {
                ...
                "txpwr": txpwr_i, "mfg": mfg_resolved, "adv_len": adv_len_i,
                "has_services": has_services_i, "services_16": services_list or [],
                "n_services_128": n_services_128_i, "mfg_data": mfg_data or "",
                "payload_sig": payload_sig,
                "last_seen_mono": now_mono,
                "last_seen_str": now_dt.strftime("%H:%M:%S"), "last_ts": ts_epoch_i,
                "scanner": scanner
            }
        ...
        with open(self.csv_log, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([
                now_dt.isoformat(), mac, name, rssi_i, channel_i, txpwr_i, mfg_resolved,
                adv_len_i, has_services_i, len(services_list or []),
                n_services_128_i, mfg_data or "", scanner, ts_epoch_i, payload_sig
            ])
...
class BLEPopupApp:
    ...
    def refresh_ui(self):
        ...
        devices_snapshot = self.model.snapshot_devices()
        self.lbl_present.config(text=f"Active: {len(devices_snapshot)}")
        current_rows = set(self.tree.get_children())
        active_keys = set()

        if self.view_mode == "SIGNALS":
            sorted_items = sorted(devices_snapshot.items(), key=lambda x: x[0])
            ...
            for mac, d in sorted(devices_snapshot.items(), key=lambda x: x[0]):
                services_text = resolve_service_names(d.get("services_16"))
                vals = (mac, d["mfg"], d["name"], services_text, d["last_seen_str"])
                if mac in current_rows: self.tree.item(mac, values=vals)
                else: self.tree.insert("", "end", iid=mac, values=vals)
                active_keys.add(mac)
    ...
    def on_close(self):
        self.stop_flag.set()
        if self.json_out:
            meta = {"start": self.start_iso, "devices": len(self.model.snapshot_devices()), "end": datetime.now().isoformat()}
